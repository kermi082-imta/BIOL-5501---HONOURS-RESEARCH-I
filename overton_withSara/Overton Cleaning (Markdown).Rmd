...
EDIT: Changed the source column that created pub.date column. 
...
```
*ASK SARA!! - Why'd we do this??
Uses an IF-ELSE function!!

##KHRIS ACTION:

SEPARATE - "Published on date of citing document" column into something like "cited.pub.year". KEEP ONLY THE YEAR. 
CHANGE IF-ELSE statement. 
Use the is.na function to search for them. 
...
# Write Out CSVs
```{r}
write_delim(missing.latin.final, "overton_withSara/missingLatinupdate.csv", delim = ',')
write_delim(missing.common.final, "overton_withSara/missingCommonupdate.csv", delim = ',')
write_delim(dup.years, "overton_withSara/dupYearsupdate.csv", delim = ',')
write_delim(final.clean, "overton_withSara/cleanDBupdate.csv", delim = ',')
```