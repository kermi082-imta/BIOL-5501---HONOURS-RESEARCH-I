...
combined_data <- sheet_names %>%
  set_names() %>% 
  map_dfr(~{
    df <- read_excel(path = file, sheet = .x, col_types = c("text","text","text","text","text","text","text", "numeric", "numeric")) # read in each sheet
    df %>% 
...
```{r}
left_combined_final_df <- left_join(sent_mostcleanestdb,
 sent_renamed_final_clean_df,
  by = c("source", "journal.pub.year"))
View(left_combined_final_df)

test <- combined_final_df %>%
  filter(!is.na(common))
...
```{r}
write_delim(sent_mostcleanestdb, "khris_cleanestdata.csv", delim = ',')
write_delim(sent_renamed_final_clean_df, "christie_cleanestdata.csv", delim = ',')
```
